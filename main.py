...
@app.route('/')
def home():
    # log.append(f"[Info] [{request.remote_addr}] [{time.asctime()}] User Visited on your Website\n")
        ...
        if int(len(value)) > int(MAX_VALUE_RATE):
            if verify['sentiment'] == True and verify['spam'] == False and verify['language'] == 'english':
                ...
                if len(str(for_database)) > MAX_VALUE_RATE:
                    ...
                    db.session.add(data)
                    db.session.commit()
                    #log.append(f"[Info] [{request.remote_addr}] [{time.asctime()}] DataBase OverWrited\n")
                    # logger.log(log)
                else:
                    flash("Length Error")
                    # log.append( f"[Error] [{request.remote_addr}] [{time.asctime()}] Length Of the sentence is Too short\n")
                    # logger.log(log)
            else:
...
def data_download():
    ...
    if request.method == "POST":
        try:
            os.mkdir(os.path.join(path(os.path.abspath(os.path.dirname(__file__))), "Data_Collecter", "data"))
        except Exception:
            pass
        ...
        if (username == login['username'] and password == login['password']):
            ...
            conn = connect(os.path.join(basedir, "database.db"))
